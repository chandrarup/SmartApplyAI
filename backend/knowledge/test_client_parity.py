#!/usr/bin/env python3
"""Verify knowledge.client parity in local and service-backed modes."""
...
from knowledge import client, store  # noqa: E402
...
def _load_source_json() -> dict[str, Any]:
...
def test_json_vs_get_profile() -> bool:
    ...
    loaded = client.get_profile("default")
    diffs = _diff("root", source, loaded)
    if diffs:
        print("FAIL: profiles/default/master_data.json vs client.get_profile('default')")
        for line in diffs[:40]:
            print(" ", line)
        if len(diffs) > 40:
            print(f"  ... and {len(diffs) - 40} more diff lines")
        return False
    print("PASS: profiles/default/master_data.json vs client.get_profile('default')")
    return True


def test_round_trip() -> bool:
    source = _load_source_json()
    pid = "__client_parity_roundtrip__"
    try:
        client.save_profile(pid, source)
        roundtripped = client.get_profile(pid)
        diffs = _diff("root", source, roundtripped)
        if diffs:
            print("FAIL: client.save_profile → client.get_profile round-trip")
            for line in diffs[:40]:
                print(" ", line)
            if len(diffs) > 40:
                print(f"  ... and {len(diffs) - 40} more diff lines")
            return False
        print("PASS: client.save_profile → client.get_profile round-trip")
        return True
    finally:
