...
Read-only inventory of how profile (“knowledge”) data is shaped, loaded, mutated, and consumed. Source of truth at runtime is **`backend/profiles/{profile_id}/master_data.json`**, with **`backend/master_data.json`** as legacy fallback.
...
`load_pdata(pid)` reads `profiles/{pid}/master_data.json`; on failure falls back to `backend/master_data.json`; on total failure returns `{}`.
...
|----------|------|-------|-------|
| `load_pdata(pid)` | `profiles/{pid}/master_data.json` → fallback `backend/master_data.json` | — | Primary loader |
| `save_pdata(pid, data)` | — | `profiles/{pid}/master_data.json` | Full-document replace |
